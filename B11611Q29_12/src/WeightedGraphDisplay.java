...
public class WeightedGraphDisplay extends Application {
	
	// Revise definition of Class GraphView to display weighted g 
	    		...
	    		for (int j = 0; j < neighbors.size(); j++) {
	    			...
	    			try {
	    				getChildren().addAll(
							...
							new Text((x1 + x2) / 2, (y1 + y2) / 2, ((WeightedGraph)graph).getWeight(i, v) + "")
						);
	    			} catch (Exception e) {
	    				e.printStackTrace();
	    			}
	...
	private WeightedGraph<City> g = new WeightedGraph<City>(vertices, edges);
	private GraphView view = new GraphView(g);
	
	@Override // Override the start method in the Application class
	public void start(Stage stage) {
		// Create a scene and place it in the stage
		Scene scene = new Scene(view, 800, 600);
		stage.setTitle("Weighted Graph Display"); // Set the stage title
